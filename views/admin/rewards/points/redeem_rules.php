<?php
// public_html/views/admin/rewards/points/redeem_rules.php
...
if(!$bootstrap_ok){ echo "<h1>Points – Redeem Rules</h1><div style='color:red;'>$bootstrap_warning</div>"; exit; }

/* Auth */
$user=$_SESSION['user']??null; if(!$user){ header('Location: /views/auth/login.php'); exit; }
$tenantId=(int)$user['tenant_id']; $userId=(int)$user['id'];

$page_title="Rewards · Points · Redeem Rules";
...
<div class="container mt-4">
  ...
  <h1 class="mb-2">Points · Redeem Rules</h1>
  <p class="text-muted">Define how points convert to value and any eligibility constraints.</p>

  <?php points_tabs('redeem'); ?>

  <form id="redeem-rules-form" method="post" action="/controllers/admin/rewards/points/rules_save.php">
    ...
    <div class="card shadow-sm">
      <div class="card-body">
        ...
        <p class="text-muted mt-2 mb-3">Stored in <code>loyalty_programs.points->redeem_rules</code>.</p>
        <textarea name="rules_json" class="form-control" rows="16" placeholder='{ "value_per_point": 0.02, "min_points": 50, "step": 10 }'></textarea>
      </div>
    </div>
  </form>
</div>

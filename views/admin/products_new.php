<?php
// views/admin/products_new.php — New Product (supports multi-value modifiers)
...
if ($bootstrap_ok){
  try {
    ...
    try {
      ...
      if (!$branches) { $branches = $pdo->query("SELECT id, title AS name FROM branches ORDER BY title ASC")->fetchAll() ?: []; }
...
<head>
<meta charset="utf-8">
<title>New Product · Smorll POS</title>
...
<div class="container">
  ...
  <form id="productForm" method="post" action="/controllers/admin/products_save.php" enctype="multipart/form-data">
    ...
    <div class="section"><div class="h1">New Product</div></div>

    <div class="section">
      <div class="h2">Basics</div>
      <div class="grid">
        <div><label>Name (English)</label><input class="input" name="name_en" required maxlength="200" placeholder="e.g., Chicken Caesar"></div>
        <div><label>Arabic Name (الاسم العربي)</label><input class="input" name="name_ar" maxlength="200" dir="rtl" placeholder="اكتب الاسم هنا"></div>
      </div>
      <div class="grid">
        <div><label>Description (English)</label><textarea class="input" name="description" rows="4" placeholder="Optional"></textarea></div>
        <div><label>Arabic Description (الوصف)</label><textarea class="input" name="description_ar" rows="4" dir="rtl" placeholder="اختياري"></textarea></div>
      </div>
      <div class="grid">
        <div>
    ...
    <div class="section">
      ...
      <div class="grid">
        <div><label>Price</label><input id="priceInput" class="input" name="price" inputmode="decimal" placeholder="0.00" value="0.00"><div class="small" id="priceHint">Enter the price unless Open Price is enabled.</div></div>
        <div><label>Standard Cost</label><input id="costInput" class="input" name="standard_cost" inputmode="decimal" placeholder="0.00" value="0.00"></div>
      </div>
      <div class="switch" style="margin-top:10px">
        <label style="margin:0"><input id="openPrice" type="checkbox" name="is_open_price"> Open Price</label>
    ...
    <div class="section">
      ...
      <div class="grid3">
        <div><label>Weight (kg)</label><input class="input" name="weight_kg" inputmode="decimal" placeholder="e.g., 0.250"></div>
        <div><label>Calories</label><input class="input" name="calories" inputmode="numeric" placeholder="e.g., 420"></div>
        <div><label>Prep time (min)</label><input class="input" name="prep_time_min" inputmode="numeric" placeholder="e.g., 15"></div>
      </div>
    </div>
...
function makeTagSelect(root, items, {multi=true, onChange}={}) {
  ...
  root.innerHTML = `
    <div class="chips"></div>
    <div class="dropdown"><input type="text" placeholder="Type to search…"><div class="menu"></div></div>`;
  const chips=root.querySelector('.chips'), input=root.querySelector('input'), menu=root.querySelector('.menu');
  function renderChips(){ chips.innerHTML=''; state.selected.forEach(it=>{ const s=document.createElement('span'); s.className='chip'; s.textContent=it.name; const x=document.createElement('button'); x.type='button'; x.textContent='×'; x.addEventListener('mousedown',e=>e.preventDefault()); x.onclick=()=>{state.selected.delete(it.id); renderChips(); renderMenu(); onChange && onChange(getSelected());}; s.appendChild(x); chips.appendChild(s); }); }
  function filtered(){ const q=input.value.trim().toLowerCase(); return items.filter(it=>!state.selected.has(it.id) && (q==='' || (it.name||'').toLowerCase().includes(q))); }
  function renderMenu(){ const list=filtered(); menu.innerHTML=''; if(!list.length){const d=document.createElement('div'); d.className='empty'; d.textContent='No matches'; menu.appendChild(d); return;} list.forEach((it,i)=>{ const r=document.createElement('div'); r.className='opt'+(i===state.idx?' active':''); r.textContent=it.name; r.addEventListener('mousedown',e=>{e.preventDefault(); select(it)}); r.addEventListener('touchstart',e=>{e.preventDefault(); select(it)},{passive:false}); r.onmouseenter=()=>{state.idx=i; renderMenu();}; menu.appendChild(r); }); }
  ...
  function select(it){ if(!state.multi) state.selected.clear(); state.selected.set(it.id,it); input.value=''; renderChips(); renderMenu(); if(!state.multi) close(); onChange && onChange(getSelected()); input.focus(); }
  function getSelected(){ return Array.from(state.selected.values()); }
  input.addEventListener('focus', open);
  input.addEventListener('input', ()=>{ open(); renderMenu(); });
  input.addEventListener('keydown', (e)=>{ const list=filtered(); if(e.key==='ArrowDown'){e.preventDefault(); state.idx=Math.min(list.length-1, state.idx+1); renderMenu();} else if(e.key==='ArrowUp'){e.preventDefault(); state.idx=Math.max(0, state.idx-1); renderMenu();} else if(e.key==='Enter'){e.preventDefault(); if(state.idx>=0 && list[state.idx]) select(list[state.idx]);} else if(e.key==='Backspace' && input.value==='' && state.selected.size){ const lastId=Array.from(state.selected.keys()).pop(); state.selected.delete(lastId); renderChips(); renderMenu(); onChange && onChange(getSelected()); } });
  ...
  return {
    ...
    setSelected(list){ state.selected.clear(); list.forEach(it=>state.selected.set(it.id,it)); renderChips(); renderMenu(); onChange && onChange(getSelected()); }
  };
}

const tsCategories = makeTagSelect(document.getElementById('ts-categories'), (Array.isArray(DATA.categories)?DATA.categories:[]).map(c=>({id:Number(c.id), name:c.name})));
const tsBranches   = makeTagSelect(document.getElementById('ts-branches'),   (Array.isArray(DATA.branches)?DATA.branches:[]).map(b=>({id:Number(b.id), name:b.name})));

/* Modifiers: group single, values multi (default select all) */
const modRows = document.getElementById('modRows'); let modRowSeq=0;
...
function addModRow(groupId=null, valueIds=null){
  ...
  const wrap=document.createElement('div'); wrap.className='mod-row'; wrap.dataset.rowId=String(rowId);
  wrap.innerHTML=`
    <div><label>Modifier</label><div id="ts-modgroup-${rowId}" class="tagsel" data-multi="0"></div></div>
    <div><label>Values</label><div id="ts-modvalues-${rowId}" class="tagsel" data-multi="1"></div></div>
...
/* Submit: emit multiple (group_id, option_id) pairs; empty option if none */
document.getElementById('productForm').addEventListener('submit', function(){
  ...
  document.querySelectorAll('.mod-row').forEach(row=>{
    const selG=row.__tsG?.getSelected()||[]; if(!selG.length) return;
    const gid = selG[0].id;
...
if (imgInput){
  imgInput.addEventListener('change', function(){
    ...
    const prev=document.getElementById('imgPrev'), prevImg=document.getElementById('imgPrevImg'), prevName=document.getElementById('imgPrevName');
...
/* Open Price: CLEAR then disable (as requested) */
...
function applyOpenPriceUI(){
  ...
  if (on) { if (priceInput) priceInput.value=''; if (costInput) costInput.value=''; }
